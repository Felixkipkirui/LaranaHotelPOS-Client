...
Public Class Sales
    ...
    Private Sub Display()
        Con.Open()
        Dim query = "SELECT * FROM orders ORDER BY ID DESC"
    ...
    Private Sub btnFilter_Click(sender As Object, e As EventArgs) Handles btnFilter.Click
        ...
        Dim Sumquery As String = "SELECT SUM(Total) FROM orders WHERE Date BETWEEN '" & sformattedDate & "' AND '" & eformattedDate & "' "
        Dim maxquery As String = "SELECT MAX(Total) FROM orders WHERE Date BETWEEN '" & sformattedDate & "' AND '" & eformattedDate & "' "
        Dim minquery As String = "SELECT MIN(Total) FROM orders WHERE Date BETWEEN '" & sformattedDate & "' AND '" & eformattedDate & "' "
        Dim avgquery As String = "SELECT AVG(Total) FROM orders WHERE Date BETWEEN '" & sformattedDate & "' AND '" & eformattedDate & "' "
        ...
        Try
            ...
            Dim query = "SELECT * FROM orders WHERE Date BETWEEN '" & sformattedDate & "' AND '" & eformattedDate & "' "
            Dim adapter As SqlDataAdapter
            Dim cmd = New SqlCommand(query, Con)
            adapter = New SqlDataAdapter(cmd)
            Dim builder = New SqlCommandBuilder(adapter)
